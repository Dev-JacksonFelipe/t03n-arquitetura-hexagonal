...
import com.fag.lucasmartins.arquitetura_software.infrastructure.adapters.in.messaging.pedido.exceptions.PedidoSqsConsumerException;
...
public class SqsPedidoAdapter {
    ...
    public void receberMensagem(PedidoEventDTO evento) {
        try {
            log.info("Evento de pedido recebido do cliente {}", evento.getCustomerId());
            ...
            log.info("Pedido {} consumido com sucesso.", pedidoCriado.getId());
        } catch (Exception e) {
            log.error("Erro ao processar evento de pedido do cliente {}", evento.getCustomerId(), e);
            throw new PedidoSqsConsumerException("erro ao processar evento de pedido do cliente " + evento.getCustomerId(), e);
        }
    }
}
